...
import {
    GET_SCANNER_REPORT, QUARK_ENGINE_REPORT
...
export function QuarkEngineReportPage({reportId}: Readonly<ImplReportPageProps>) {
    const {
        loading: reportsLoading,
        data: reportsData,
    } = useQuery(GET_SCANNER_REPORT, {
        variables: {reportObjectId: reportId, wantQuarkEngine: true},
        skip: !reportId,
    });
    ...
    const reports = (reportsData?.apk_scanner_report_list ?? [])
        ...
        .map(report => useFragment(QUARK_ENGINE_REPORT, report?.androidAppIdReference.quarkEngineReport))
        .filter(isNonNullish);
