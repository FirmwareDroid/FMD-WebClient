...
export function APKScanReportPage({reportId}: Readonly<ImplReportPageProps>) {
    const {
    ...
    const reports = (reportsData?.apk_scanner_report_list ?? [])
        ...
        .map(report => useFragment(APKSCAN_REPORT, report?.androidAppIdReference.apkscanReport))
        .filter(isNonNullish);
