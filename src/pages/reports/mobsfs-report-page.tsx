...
import {
    GET_SCANNER_REPORT, MOBSFS_REPORT
...
export function MobSFSReportPage({reportId}: Readonly<ImplReportPageProps>) {
    const {
        loading: reportsLoading,
        data: reportsData,
    } = useQuery(GET_SCANNER_REPORT, {
        variables: {reportObjectId: reportId, wantMobsfs: true},
        skip: !reportId,
    });
    ...
    const reports = (reportsData?.apk_scanner_report_list ?? [])
        ...
        .map(report => useFragment(MOBSFS_REPORT, report?.androidAppIdReference.mobsfReport))
        .filter(isNonNullish);
