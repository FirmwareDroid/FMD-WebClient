...
export function FlowdroidReportPage({reportId}: Readonly<ImplReportPageProps>) {
    const {
    ...
    const reports = (reportsData?.apk_scanner_report_list ?? [])
        ...
        .map(report => useFragment(FLOWDROID_REPORT, report?.androidAppIdReference.flowdroidReport))
        .filter(isNonNullish);
