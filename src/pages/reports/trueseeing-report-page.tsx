...
import {
    GET_SCANNER_REPORT, TRUESEEING_REPORT
...
export function TrueseeingReportPage({reportId}: Readonly<ImplReportPageProps>) {
    const {
        loading: reportsLoading,
        data: reportsData,
    } = useQuery(GET_SCANNER_REPORT, {
        variables: {reportObjectId: reportId, wantTrueseeing: true},
        skip: !reportId,
    });
    ...
    const reports = (reportsData?.apk_scanner_report_list ?? [])
        ...
        .map(report => useFragment(TRUESEEING_REPORT, report?.androidAppIdReference.trueseeingReport))
        .filter(isNonNullish);
