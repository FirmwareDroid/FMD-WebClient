...
import {
    GET_SCANNER_REPORT, SUPER_REPORT
...
export function SuperReportPage({reportId}: Readonly<ImplReportPageProps>) {
    const {
        loading: reportsLoading,
        data: reportsData,
    } = useQuery(GET_SCANNER_REPORT, {
        variables: {reportObjectId: reportId, wantSuper: true},
        skip: !reportId,
    });
    ...
    const reports = (reportsData?.apk_scanner_report_list ?? [])
        ...
        .map(report => useFragment(SUPER_REPORT, report?.androidAppIdReference.superReport))
        .filter(isNonNullish);
